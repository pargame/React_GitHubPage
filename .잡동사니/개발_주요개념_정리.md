...
### CI/CD 구현 경로
- CI/CD는 GitHub Actions를 통해 `.github/workflows/<workflow-name>.yml` 파일로 구현됩니다.
- 이 파일은 지속적 통합(CI)과 지속적 배포(CD)를 자동화하는 데 사용됩니다.

#### CI와 CD의 역할
1. **CI (Continuous Integration)**:
   - 코드 변경 시 자동으로 테스트, 빌드, 린트 등을 실행합니다.
   - 코드 품질을 유지하고, 변경 사항이 기존 코드와 잘 통합되는지 확인합니다.

2. **CD (Continuous Deployment)**:
   - 빌드된 결과물을 자동으로 배포합니다.
   - 예를 들어, React 앱을 GitHub Pages로 배포하거나 클라우드 서비스에 배포할 수 있습니다.

#### `.yml` 파일에서 CI/CD 구현
- **CI 단계**:
  - 코드 체크아웃, 의존성 설치, 테스트 실행 등을 포함합니다.
- **CD 단계**:
  - 빌드된 결과물을 배포 대상(예: GitHub Pages, AWS, Netlify 등)에 업로드합니다.

#### 예시: React 앱의 CI/CD 워크플로
```yaml
...
## 10. VS Code의 User와 Workspace 설정

### User 설정
- **User 설정**은 VS Code 전체에서 적용되는 글로벌 설정입니다.
- 모든 프로젝트와 폴더에 동일하게 적용됩니다.
- 설정 파일 위치:
...
### Workspace 설정
- **Workspace 설정**은 특정 프로젝트나 폴더에만 적용되는 로컬 설정입니다.
- User 설정보다 우선시됩니다.
- `.vscode/settings.json` 파일에 저장됩니다.
- 예시:
  ```json
  {
    "editor.tabSize": 4,
...
### 우선순위
1. **Workspace 설정**: 특정 프로젝트에만 적용되며, User 설정을 덮어씁니다.
2. **User 설정**: 글로벌 설정으로, Workspace 설정이 없는 경우 적용됩니다.

### 활용 예시
- 프로젝트마다 다른 탭 크기나 자동 저장 방식을 설정하고 싶을 때 Workspace 설정을 사용합니다.
- 모든 프로젝트에서 동일한 동작을 원할 경우 User 설정을 사용합니다.
